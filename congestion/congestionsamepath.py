...
def install_proactive(topo):
	...
	# ARP
	for i in range(1, 4):
		cmd = "ovs-ofctl add-flow s3 -O OpenFlow13 'table=0,idle_timeout=0,hard_timeout=0,priority=1,arp,nw_dst=10.0.0.%d,actions=output:1'" % i
		os.system(cmd)
	for i in range(4, 7):
		cmd = "ovs-ofctl add-flow s3 -O OpenFlow13 'table=0,idle_timeout=0,hard_timeout=0,priority=1,arp,nw_dst=10.0.0.%d,actions=output:2'" % i
		os.system(cmd)
	# IP
	for i in range(1, 4):
		cmd = "ovs-ofctl add-flow s3 -O OpenFlow13 'table=0,idle_timeout=0,hard_timeout=0,priority=1,ip,nw_dst=10.0.0.%d,actions=output:1'" % i
		os.system(cmd)
	for i in range(4, 7):
		cmd = "ovs-ofctl add-flow s3 -O OpenFlow13 'table=0,idle_timeout=0,hard_timeout=0,priority=1,ip,nw_dst=10.0.0.%d,actions=output:2'" % i
		os.system(cmd)
...
def traffic_generation(net, topo):
	...
	monitor = Process(target = monitor_devs_ng, args = ('./results/samepath/bwmng.txt', 1.0))
	monitor.start()

	# 2. Start ping. (Mouse flows)
	h4 = net.get('h4')
	h1 = net.get('h1')
	h1.cmd("ping -c 120 -n %s > ./results/samepath/ping.txt &" % h4.IP())
