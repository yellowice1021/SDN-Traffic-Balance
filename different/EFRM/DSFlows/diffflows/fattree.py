...
def install_proactive(net, topo):
	...
	for sw in topo.EdgeSwitchList:
		...
		# 	for j in xrange(2):
		# 		cmd = "ovs-ofctl add-flow %s -O OpenFlow13 \
		# 		'table=0,priority=10,arp,nw_dst=10.%d.0.%d,actions=group:1'" % (sw, i + 1, j + 1)
		# 		os.system(cmd)
		# 		cmd = "ovs-ofctl add-flow %s -O OpenFlow13 \
		# 		'table=0,priority=10,ip,nw_dst=10.%d.0.%d,actions=group:1'" % (sw, i + 1, j + 1)
		# 		os.system(cmd)
...
def monitor_devs_ng(fname="../data/diffflows.txt", interval_sec=0.1):
	"""
...
def traffic_generation(net, topo, flows_peers):
	...
	for src, dest in flows_peers:
		...
		if it > number:
			...
			all_mice_flows.append((src, dest))
		it = it + 1
	...
	for src, dest in elephant_flows:
		server = net.get(dest)
		client = net.get(src)
		client.cmd("iperf -c %s -b 5M -t %d > /dev/null &" % (server.IP(), 1990))
		if i < 2:
			time.sleep(5)
			i = i + 1
		else:
		...
		for s in xrange(6):
			...
			client = net.get(src)
			server = net.get(dest)
			client.cmd("ping -c %d -i 0.1 -n -q %s >> %s/%s &" % (100, server.IP(), output_dir, 'successive_packets.txt'))
	# Wait for the traffic to become stable.
	# time.sleep(10)

	# 2. Start bwm-ng to monitor throughput.
	monitor = Process(target = monitor_devs_ng, args = ('%s/bwmng.txt' % output_dir, 1.0))
	monitor.start()
