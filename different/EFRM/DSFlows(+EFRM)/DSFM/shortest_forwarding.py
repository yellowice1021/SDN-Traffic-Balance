...
class ShortestForwarding(app_manager.RyuApp):
    ...
    def get_path(self, src, dst, ip_src, ip_dst):
        ...
        """
        main_path = self.monitor.find_bandwidth_delay_path(ip_src, ip_dst)
        ...
        # self.monitor.elephant_path[(ip_src, ip_dst)] = main_path
        ...
        if isinstance(ip_pkt, ipv4.ipv4):
            ...
            if len(pkt.get_protocols(ethernet.ethernet)):
                ...
                if isinstance(udp_pkt, udp.udp):
                    ...
                    if (src, dst) not in self.monitor.elephant_src:
                        self.monitor.elephant_src.append((src, dst))
                else:
                    self.shortest_forwarding(msg, eth_type, ip_pkt.src, ip_pkt.dst)
