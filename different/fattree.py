...
def traffic_generation(net, topo, flows_peers):
	...
	for flow in flows_peers:
		if number < 3:
			if flow not in elephant_flows:
				elephant_flows.append(flow)
				number = number + 1

	serversList = set([peer[1] for peer in elephant_flows])
	for server in serversList:
		path = "../result/random/dynamic/1/" + server + ".txt"
		server = net.get(server)
		server.cmd("iperf -s -u -i 1 > %s &" % (path))
	...
	for src, dest in elephant_flows:
		server = net.get(dest)
		client = net.get(src)
		client.cmd("iperf -c %s -b 60M -t %d > /dev/null &" % (server.IP(), 60))
		mice_flows = random.sample(flows_peers, 10)
		# for client, server in mice_flows:
		# 	client = net.get(client)
		# 	server = net.get(server)
		# 	if client != server:
		# 		client.cmd("ping -c %d -i 0.6 -n -q %s >> %s &" % (
		# 			60, server.IP(), '../result/random/dynamic/1/successive_packets.txt'))
		time.sleep(1)
...
def run_experiment(pod, density, ip="127.0.0.1", port=6661, bw_c2a=100, bw_a2e=100, bw_e2h=100):
	"""
	...
	time.sleep(15)
	traffic_generation(net, topo, peers.peers1)
