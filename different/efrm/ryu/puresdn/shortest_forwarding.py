...
class ShortestForwarding(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.elephant_info = []
        self.flow_request = 0
        self.weight = self.WEIGHT_MODEL['hop']
    ...
    def get_path(self, src, dst, ip_src, ip_dst, type):
        """
        ...
        if type == 'bandwidth':
            ...
            main_path = self.monitor.get_path_delay(shortest_paths.get(src).get(dst), ip_src, ip_dst)
    ...
    def send_flow_mod(self, datapath, flow_info, src_port, dst_port, flow_tcp):
        """
    ...
    def install_flow(self, datapaths, link_to_port, access_table, path,
        ...
        if len(path) > 2:
            for i in xrange(1, len(path)-1):
                ...
                if port and port_next:
                    ...
                    self.send_flow_mod(datapath, flow_info, src_port, dst_port, 'dst')
                    self.send_flow_mod(datapath, back_info, dst_port, src_port, 'src')
        ...
        if len(path) > 1:
            ...
            self.send_flow_mod(last_dp, flow_info, src_port, dst_port, 'dst')
            self.send_flow_mod(last_dp, back_info, dst_port, src_port, 'src')
            ...
            self.send_flow_mod(first_dp, flow_info, in_port, out_port, 'dst')
            self.send_flow_mod(first_dp, back_info, out_port, in_port, 'src')
    ...
    def shortest_forwarding(self, msg, eth_type, ip_src, ip_dst, type):
        """
        ...
        if result:
            ...
            if dst_sw:
                ...
                path = self.get_path(src_sw, dst_sw, ip_src, ip_dst, type)
            ...
            if len(pkt.get_protocols(ethernet.ethernet)):
                ...
                if isinstance(udp_pkt, udp.udp):
                    if (src, dst) not in self.elephant_info:
                        self.shortest_forwarding(msg, eth_type, ip_pkt.src, ip_pkt.dst, 'bandwidth')
                        self.elephant_info.append((src, dst))
                        self.flow_request = self.flow_request + 1
                        self.logger.info(self.flow_request)
                        if len(self.elephant_info) > 1:
                            self.elephant_info.pop(0)
                else:
                    self.shortest_forwarding(msg, eth_type, ip_pkt.src, ip_pkt.dst, 'delay')
                    self.flow_request = self.flow_request + 1
                    self.logger.info(self.flow_request)
