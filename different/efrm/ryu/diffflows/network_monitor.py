        ...
        for datapath in self.datapaths.values():
            ...
            if src.startswith('3') or src.startswith('2'):
                ...
                for i in xrange(2):
                    port = i + 1
                    src = datapath.id
                    dst = self.awareness.link_in_to[(src, port)]
                    free = self.free_bandwidth[src][port]
                ...
                #     port = i + 1
                #     if delay != 0:
                #         weight[(src, port)] = 100 - int(round(weight[(src, port)] / delay, 2) * 100)
                #     else:
                #         weight[(src, port)] = 50
                # for key in weight:
                #     if delay != 0:
                #         weight[key] = int(round(weight[key] / data, 2) * 100)
                # if weight[(src, 1)] > weight[(src, 2)]:
                #     weight[(src, 1)] = 0
                #     weight[(src, 2)] = 100
                # else:
                #     weight[(src, 1)] = 100
