...
def create_stag_peers(HostList, edge_prob, pod_prob, flows_num_per_host):
	...
	for host in HostList:
		for i in xrange(flows_num_per_host):
			peer = random.choice(HostList)
			while (peer == host) or (host, peer) in peers or (peer, host) in peers:
				peer = random.choice(HostList)
...
def create_stride_peers(HostList, flow_num, stride_num):
	...
	for host in HostList:
		for i in xrange(flow_num):
			if number <= 7:
				peers_number = (HostList.index(host) + 8) % 16
			else:
				peers_number = (HostList.index(host) + 4) % 16
			number = number + 1
	...
	for host in HostList:
		...
		podList = create_podList(num)
		new_peers = []
		while len(new_peers) < flow_num:
			probability = random.random()
			if probability < 0.2:
				peer = random.choice(swList)
				if (peer != host) and ((host, peer) not in peers):
					new_peers.append((host, peer))
			elif 0.2 <= probability < 0.2 + 0.3:
				peer = random.choice(podList)
				if (peer not in swList) and ((host, peer) not in peers):
					new_peers.append((host, peer))
			else:
				peer = random.choice(HostList)
				if (peer not in podList) and ((host, peer) not in peers):
					new_peers.append((host, peer))
		peers.extend(new_peers)
...
def create_peers(host_num, flow_num):
	...
	# peers = create_stride_peers(HostList, flow_num, 8)
	# file_save = open('peers.py', 'w')
	# file_save.write('peers=%s' % peers)
	# file_save.close()
