...
def traffic_generation(net, topo, flows_peers):
	...
	for src, dest in elephant_flows:
		...
		client.cmd("iperf -c %s -b 2M -t %d > /dev/null &" % (server.IP(), 1990))   # Its statistics is useless, just throw away. 1990 just means a great number.
		time.sleep(3)
		if j < 10:
			for k in xrange(2):
				p = mice_flows[i]
