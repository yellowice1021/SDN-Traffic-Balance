...
def traffic_generation(net, topo, flows_peers):
	...
	for src, dest in elephant_flows:
		...
		client.cmd("iperf -c %s -b 9M -t %d > /dev/null &" % (server.IP(), 1990))   # Its statistics is useless, just throw away. 1990 just means a great number.
		time.sleep(6)
	...
	for src, dest in mice_flows:
		client = net.get(src)
		server = net.get(dest)
		client.cmd("ping -c %d -i 0.1 -n -q %s >> %s/%s &" % (600, server.IP(), '../data/ecmp/9/5', 'successive_packets.txt'))
