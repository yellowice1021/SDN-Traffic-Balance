...
class NetworkMonitor(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.monitor_link_thread = hub.spawn(self._monitor_links)
    ...
    def _monitor_links(self):
        """
            collect flow statistics and dynamic scheduling flows
        """
        while True:
            self.elephant_info = {}
            for dp in self.datapaths.values():
    ...
    def link_weight_group(self):
        """
            setting group table weight
        """
        for datapath in self.datapaths.values():
            src = str(datapath.id)
            if src.startswith('3') or src.startswith('2'):
                weight = {}
                data = 0
                bandwidth = 0
                for i in xrange(2):
                    port = i + 1
                    src = datapath.id
                    free = self.port_info[(src, port)]
                    # weight[(src, port)] = self.awareness.graph[src][dst]['delay']
                    weight[(src, port)] = (free / setting.MAX_CAPACITY) * 100
                    bandwidth = (free / setting.MAX_CAPACITY) * 100
                for i in xrange(2):
                    weight[(src, port)] = round(weight[(src, port)] / bandwidth) * 100
                # self.logger.info(weight)
                self.send_group_mod(datapath.id, weight)

    def send_group_mod(self, datapath, weight):
        weight_1 = weight[(datapath, 1)]
        weight_2 = weight[(datapath, 2)]
        datapath = self.datapaths[datapath]
    ...
    def _request_flow_stats(self, datapath):
        """
            sending request to switch to collect flow statistics
        """
        self.logger.debug('send stats request: %016x', datapath.id)
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
    ...
    def send_flow_mod(self, datapath, flow_info, src_port, dst_port, flow_tcp, priority):
        ...
        self.add_flow(datapath, priority + 1, match, actions,
                      idle_timeout=160, hard_timeout=0)
        ...
        if(len(paths) > 1):
            for path in paths:
                ...
                for i in xrange(path_length - 1):
                    ...
                    max_bandwidth = max(max_bandwidth, link_left_bandwidth)

                score = 0.7 * ((setting.MAX_CAPACITY - min_bandwidth) / setting.MAX_CAPACITY) + 0.3 * ((max_bandwidth - min_bandwidth) / setting.MAX_CAPACITY)
                if score < min_score:
                    max_path = path
                    min_score = score
    ...
    def find_new_path(self, src, dst, link_flow, choose_path):
        """
    ...
    def find_max_bandwidth(self, src, dst, speed, score_number, score_path):
        """
        ...
        if ifPath:
            max_score = setting.MAX_CAPACITY / (max_bandwidth + speed)
            score_number[(src, dst)] = max_bandwidth_path
            score_path[(src, dst)] = max_score

    def link_flow_set(self, choose_flow):
        """
            give flow a sort number
        """
        score_path = {}
        score_number = {}
        score_path_flow = {}
        score_flow = []
        all_flows = choose_flow.keys()
        ifPath = False
        for flow in all_flows:
            (src, dst) = flow
            speed = choose_flow[(src, dst)]
            self.find_max_bandwidth(src, dst, speed, score_number, score_path)

        if score_path:
            score_path_flow = sorted(score_path.items(), key=lambda x:x[1], reverse=True)
            for flow in score_path_flow:
                score_flow.append((flow[0], score_number[flow[0]]))
        return score_flow
    ...
    def change_flow_path(self, link, src, src_port):
        ...
        if ifChoose:
            flow_set = self.link_flow_set(choose_flow)
            ...
            if flow_set:
                ...
                while bandwidth < setting.MAX_CAPACITY * 0.1:
                    if number == i:
                        break
                    flow = flow_set[number]
                    src_dst_flow = flow[0]
                    src_dst_flow_path = flow[1]
                    number = number + 1
                    src_ip = src_dst_flow[0]
                    dst_ip = src_dst_flow[1]
                    self.find_new_path(src_ip, dst_ip, link_flow, src_dst_flow_path)
                    bandwidth = self.port_info[(src, src_port)]
    ...
    def link_status(self):
        ...
        if link_number != 0:
            ...
            if monitor_time != 0:
                ...
                monitor_time = - 10 * numpy.log2(monitor_time)
