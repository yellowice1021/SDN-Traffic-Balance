...
class Fattree(Topo):
    ...
    def _addSwitch(self, number, level, switch_list):
        for x in xrange(1, number+1):
            ...
            # if x >= int(10):
            #     PREFIX = str(level) + "0"
            switch_list.append(self.addSwitch('s' + str(self.switchNumber)))
            self.switchNumber += 1
    ...
    def createHost(self, NUMBER):
        ...
        for x in xrange(1, NUMBER+1):
            ...
            # elif x >= int(100):
            #     PREFIX = "h"
            self.HostList.append(self.addHost("h" + str(self.hostNumber)))
            self.hostNumber += 1
...
def createTopo(pod, density, ip="127.0.0.1", port=6661, bw_c2a=0.2, bw_a2e=0.1, bw_h2a=0.05):
