...
def traffic_generation(net, topo, flows_peers):
	...
	monitor = Process(target=monitor_devs_ng, args=('../result/random/ecmp/2/bwmng.txt', 1.0))
	monitor.start()
	...
	for client, server in mice_flows:
		...
		server = net.get(server)
		if client != server:
			client.cmd("ping -c %d -i 0.6 -n -q %s >> %s &" % (60, server.IP(), '../result/random/ecmp/2/successive_packets.txt'))
...
def run_experiment(pod, density, bw_c2a=10, bw_a2e=10, bw_e2h=10):
	"""
