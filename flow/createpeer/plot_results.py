...
def plot_results():
	...
	bwmng_file = '../result/random/differenttype/2/bwmng.txt'
	# throughput = get_throughput(throughput, 'random', 'ECMP', bwmng_file)
	keys = ['average_round_trip_delay', 'mean_deviation_of_round_trip_delay', 'total_send', 'total_receive']
	successive_packets_file = '../result/random/differenttype/2/successive_packets.txt'
	average_delay = get_delay(average_delay, 'random', keys, 'ECMP', successive_packets_file)
	total_send = average_delay['random']['total_send']['ECMP']
	total_receive = average_delay['random']['total_receive']['ECMP']
	print(average_delay)
	print('ECMP平均往返时延：' + str(average_delay['random']['average_round_trip_delay']['ECMP']))
	print('ECMP往返时延平均方差：' + str(average_delay['random']['mean_deviation_of_round_trip_delay']['ECMP']))
	print('ECMP平均丢包率：' + str((total_send - total_receive) / float(total_send)))

	throughput = get_throughput(throughput, 'random', 'ECMP', bwmng_file)
	average_throughput = throughput['random']['accumulated_throughput']['ECMP'][args.duration] / float(args.duration)
	print('ECMP平均吞吐量：' + str(average_throughput))
