...
def create_random_peers(HostList, flow_num):
	...
	peers = []
	number = 0
	while (number < flow_num):
		number = number + 1
		src_peer = random.choice(HostList)
		dst_peer = random.choice(HostList)
		while (src_peer == dst_peer):
			dst_peer = random.choice(HostList)
		peers.append((src_peer, dst_peer))
	return peers
...
def create_peers(host_num, flow_num):
	...
	file_save.close()
	# while (len(flows_peers) == 0) or (len(set([peer[1] for peer in flows_peers])) > (host_num -2)):
